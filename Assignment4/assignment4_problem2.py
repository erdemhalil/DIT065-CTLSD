...
def extract_login_result(line):
    ...
    return 1 if "Accepted password for" in line else 0
...
if __name__ == '__main__':
    ...
    # Parse date, login results, account number and IP
    ...
    invalid_ccounts_failed = invalidf_user_lines.map(extract_invalid_account)
    ...
    # print("Invalid users:", invalid_ccounts_failed.collect())

#----------Subtask: problem(a)---------- 
    # Combine successful and unsuccessful login attempts for each user account
    combined_attempts = accounts_accepted.map(lambda account: (account, 1)).union(accounts_failed.map(lambda account: (account, 1)))
    # Calculate total attempts for each user account
    total_attempts = combined_attempts.reduceByKey(lambda a, b: a + b)
    # Find the account with the largest number of attempts
...
#----------Subtask: problem(b)----------
    ...
    total_successful_attempts = accounts_accepted.map(lambda account: (account, 1)).reduceByKey(lambda a, b: a + b)
...
#----------Subtask: problem(c)----------
    ...
    total_unsuccessful_attempts = accounts_failed.map(lambda account: (account, 1)).reduceByKey(lambda a, b: a + b)
    # Find the account with the largest number of unsuccessful attempts
...
#----------Subtask: problem(d)----------
    ...
    # Find the accounts with the highest success rate
...
#----------Subtask: problem(e)----------
    ...
    failed_login_attempts_by_ip = ips_failed.map(lambda ip: (ip, 1)).reduceByKey(lambda a, b: a + b)

    # Find the top 3 IP addresses with the largest number of failed login attempts
...
#----------Subtask: problem(f)----------
    ...
    failed_login_attempts_by_invalid_user = invalid_ccounts_failed.filter(lambda account: account != "").map(lambda account: (account, 1)).reduceByKey(lambda a, b: a + b)

    # Find the top 10 invalid user accounts with the largest number of failed login attempts
...
#----------Subtask: problem(g)----------
    ...
    combined_attempts_by_date = dates_accepted.map(lambda date: (date, 1)).union(dates_failed.map(lambda date: (date, 1)))

    # Calculate total attempts for each date
    total_attempts_by_date = combined_attempts_by_date.reduceByKey(lambda a, b: a + b)

    # Find the day with the most login activity
    max_activity_day = total_attempts_by_date.max(lambda x: x[1])

    # Find the day with the least login activity
