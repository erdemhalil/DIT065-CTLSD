...
def linear_scan(X, Q, b = None):
    ...
    for batch_idx in range(num_batches):
        ...
        X_norm_sq = cp.sum(X**2, axis=1, keepdims=True)
        Q_norm_sq = cp.sum(Q_batch**2, axis=1, keepdims=True)
        distances = X_norm_sq - 2 * cp.dot(X, Q_batch.T) + Q_norm_sq.T
        
        # Find the indices of the minimum distances
        I[start:end] = cp.argmin(distances, axis=0)
