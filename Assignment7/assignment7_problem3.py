...
def linear_scan(X, Q, b = None):
    ...
    #     Q_norm_sq = np.sum(Q_batch**2, axis=1, keepdims=True)
    #     distances = X_norm_sq - 2 * np.dot(X, Q_batch.T) + Q_norm_sq.T
        
    #     # Find the indices of the minimum distances
    #     I[start:end] = np.argmin(distances, axis=0)
    ...
    for batch_idx in range(num_batches):
        ...
        # Compute ||X||^2 for all rows in X
        X_norms_squared = np.sum(X**2, axis=1)
        # Compute ||Q_batch||^2 for all rows in Q_batch
        Q_batch_norms_squared = np.sum(Q_batch**2, axis=1)

        # Compute the inner product XQ^T
        inner_product = np.dot(Q_batch, X.T)

        # Compute the distance matrix
        distances = Q_batch_norms_squared[:, np.newaxis] + X_norms_squared - 2 * inner_product

        # Find the indices of the minimum distances
        I[start:end] = np.argmin(distances, axis=1)
