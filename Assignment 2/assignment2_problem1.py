...
def play_blackjack(strategy: strategy_type, bet: float = 1.0, seed: Optional[int] = None)->float:
    """
...
def worker_simulate(worker_seed, chunk_size, strategy):
    results = np.zeros(chunk_size)
    for i in range(chunk_size):
        results[i] = play_blackjack(strategy, seed=worker_seed)
    return results
...
def simulate(n: int,
    ...
    - num_workers: number of worker processes to use for parallelization
    - seed: optional random number seed to make the results reproducible
    """

    chunk_size = n // num_workers

    loop_start_time = time.time()  # Start time before loop
    with multiprocessing.Pool(num_workers) as pool:
        seeds = [seed + i for i in range(num_workers)]
        results = pool.starmap(worker_simulate, [(s, chunk_size, strategy) for s in seeds])
    loop_end_time = time.time()  # End time after loop

    return np.mean(results), loop_start_time, loop_end_time
...
#     loop_start_time = time.time()  # Start time before loop
#     for i in range(n):
#         results[i] = play_blackjack(basic_strategy, seed=seed)
#     loop_end_time = time.time()  # End time after loop

#     res: float = results.mean()
#     return res, loop_start_time, loop_end_time
...
if __name__ == '__main__':
    ...
    res, loop_start, loop_end = simulate(n=n, num_workers=num_workers, seed=seed)
    end = time.time() # end timepoint

    total_time = end - start
    parallelizable_time = loop_end - loop_start
    
    # Calculate P (fraction of the program that can be parallelized)
    p = parallelizable_time / total_time

    theoretical_speedup = 1 / ((1 - p) + p / num_workers)

    print(f"Fraction of the program that can be parallelized (P): {p}")
    print(f"Fraction of the program that is sequential (1 - P): {1 - p}")
    print(f"Speedup using Amdahl's law: {theoretical_speedup}")
    print(f"Upper bound:{1 / (1 - p)}")

    print(f"Total runtime: {end-start}")
    print(f"Parallelizable loop runtime: {loop_end-loop_start}")
    print(f"Sequential part runtime: {end-start-(loop_end-loop_start)}")
